    ...
    try {
        ...
        switch ($action) {
            ...
            case 'categories_list':
                echo json_encode(['ok'=>true,'data'=>$pdo->query("SELECT id, slug, nom, IFNULL(nom_es,'') AS nom_es, icona, color, ordre, activa FROM categories ORDER BY ordre")->fetchAll(PDO::FETCH_ASSOC)]);
                break;
            ...
            case 'categoria_save':
                ...
                if ($id) {
                    $pdo->prepare("UPDATE categories SET slug=?,nom=?,nom_es=?,icona=?,color=?,ordre=? WHERE id=?")->execute([$slug,$nom,$nom_es,$icona,$color,$ordre,$id]);
                } else {
                    $pdo->prepare("INSERT INTO categories (slug,nom,nom_es,icona,color,ordre) VALUES (?,?,?,?,?,?)")->execute([$slug,$nom,$nom_es,$icona,$color,$ordre]);
                    $id = (int)$pdo->lastInsertId();
            ...
            case 'peticio_estat':
                ...
                // Si es marca com a RESOLT → actualitza el recurs del barri a 'ok'
                if ($estat === 'resolt') {
                    ...
                    if ($row && $row['barri_id'] && $row['categoria_id']) {
                        $pdo->prepare("INSERT INTO recursos_barri (barri_id, categoria_id, estat)
                            VALUES (?, ?, 'ok')
                            ON DUPLICATE KEY UPDATE estat='ok', notes=CONCAT(IFNULL(notes,''), ' [Resolt petició #".$id."]')")
                            ->execute([$row['barri_id'], $row['categoria_id']]);
                    }
                }

                // Si es torna a PENDENT o PROCESS → reverteix el recurs a 'missing'
                if (in_array($estat, ['pendent', 'process'])) {
                    $pet2 = $pdo->prepare("SELECT barri_id, categoria_id FROM peticions WHERE id=?");
                    $pet2->execute([$id]);
                    $row2 = $pet2->fetch(PDO::FETCH_ASSOC);
                    if ($row2 && $row2['barri_id'] && $row2['categoria_id']) {
                        $pdo->prepare("UPDATE recursos_barri SET estat='missing'
                            WHERE barri_id=? AND categoria_id=?")
                            ->execute([$row2['barri_id'], $row2['categoria_id']]);
                    }
                }
...
<div class="modal" style="max-width:480px">
  ...
  <div class="modal-body">
    ...
    <div class="form-grid">
      ...
      <div class="fg full"><label class="fl"><?= $at["nom_lbl"] ?> (CA)</label><input class="fc" id="c-nom" placeholder="Ex: Centre de Salut"></div>
      <div class="fg full"><label class="fl"><?= $at["nom_lbl"] ?> (ES)</label><input class="fc" id="c-nom-es" placeholder="Ex: Centro de Salud"></div>
...
function openCatModal(id){
  ...
  $('c-id').value=''; $('c-nom').value=''; $('c-nom-es').value=''; $('c-slug').value=''; $('c-icona').value='📌'; $('c-ordre').value='0';
  setColor('c-color','c-color-hex','#3b82f6');
  if(id){ const c=catData.find(x=>x.id==id); if(c){$('c-id').value=c.id;$('c-nom').value=c.nom;$('c-nom-es').value=c.nom_es||'';$('c-slug').value=c.slug;$('c-icona').value=c.icona;$('c-ordre').value=c.ordre;setColor('c-color','c-color-hex',c.color);} }
  openModal('modal-cat');
}
async function saveCat(){
  const data={id:+$('c-id').value||0,nom:$('c-nom').value.trim(),nom_es:$('c-nom-es').value.trim(),slug:$('c-slug').value.trim(),icona:$('c-icona').value,color:$('c-color-hex').value,ordre:+$('c-ordre').value};
